...
module.exports = {
    ...
    createFile: function (file, tpl, project, append) {
        ...
        let code = fu.readFile(join(__dirname, '../tpl/', tpl))
            ...
            .replace(/\{\{SKETCH\}\}/g, project)
            .replace(/\{\{PREFIX\}\}/g, project.replace(/^[a-z-]/gi, '').replace(/[-]/gi, '_').toUpperCase())
            .replace(/\{\{FILENAME\}\}/g, basename(file))
        if (!append) {
            console.debug(`Create file '${file}'`)
            //cliz.debug(`Create file '${file}'`)
            fu.writeFile(file, code)
        } else {
            console.debug(`Update file '${file}'`)
